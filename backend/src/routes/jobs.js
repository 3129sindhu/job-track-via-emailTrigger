...
router.get("/", verifyToken, async (req, res) => {
  ...
  `SELECT j.*,
   ...
   LEFT JOIN gmail_messages gm
     ON gm.user_id = j.user_id AND gm.gmail_message_id = j.gmail_message_id
   ${where.replace("WHERE user_id = $1", "WHERE j.user_id = $1")}
...
const count = await pool.query(
  `SELECT COUNT(*) FROM jobs j ${where.replace("WHERE user_id = $1", "WHERE j.user_id = $1")}`,
  values
);
    res.json({
      ok: true,
